...
import { creaNuevoSegmento, checkAutoColision, reJugar } from "./functions.js";
...
window.onload = () => {
  ...
  window.mostrarHi = document.querySelector(".hi");
  ...
  setInterval(() => {
    if (!settings.gameOver) {
      ...
      if (
        headSnake.x > settings.grid[0] ||
        headSnake.x < 1 ||
        headSnake.y > settings.grid[1] ||
        headSnake.y < 1 ||
        autoColision
      ) {
        ...
        if (settings.puntos > (settings.record || 0)) {
          settings.record = settings.puntos;
          mostrarHi.innerHTML = `High:${settings.record
            .toString()
            .padStart(2, "0")}`;
        }
      }
      ...
      if (headSnake.x === manzana.x && headSnake.y === manzana.y) {
        ...
        settings.puntos++;
        mostrarPtos.innerHTML = `${settings.puntos
          .toString()
          .padStart(3, "0")}`;
      }
    }
